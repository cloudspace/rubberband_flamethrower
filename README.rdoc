...
<tt>curl -k -L -o elasticsearch-0.20.6.tar.gz http://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-0.20.6.tar.gz</tt>

Unarchive Elastic Search:

<tt>tar -zxvf elasticsearch-0.20.6.tar.gz</tt>

Start an Elastic Search node:

<tt>./elasticsearch-0.20.6/bin/elasticsearch -f</tt>
...
<tt>gem install rubberband_flamethrower</tt>

== Use

=== How to Insert into Elastic Search by using the gem through irb

First require the gem:

<tt>require 'rubberband_flamethrower'</tt>

Then you are ready to use it!

<tt>RubberbandFlamethrower.send_batch</tt>

== Changing the Defaults/Passing Parameters

The send_batch method can be configured by passing parameters to it. By default it will insert 5000 documents starting with document ID 1 into an Elastic Search index named "twitter" of type "tweet" into a server node located at http://localhost:9200.
...
<tt>RubberbandFlamethrower.send_batch(10000)</tt>

To Insert 5,000 starting with the ID 5001

<tt>RubberbandFlamethrower.send_batch(5000, 5001)</tt>

To Insert 2,000 starting with the ID 1 to a server located at http://es.com:9200

<tt>RubberbandFlamethrower.send_batch(2000, 1,"http://es.com:9200")</tt>

To put your documents into an index named "facebook" instead of "twitter" with a type of "message" instead of "tweet"

<tt>RubberbandFlamethrower.send_batch(5000, 1, "http://localhost:9200", "facebook", "message")</tt>
